...
object MgmtService {
  ...
  object Evt {
    case object Blocking extends InfoE
    case object Unblocking extends InfoE
  }
...
class MgmtService extends StatelessActor {
  ...
  onMessage {
    case Block(ps) =>
      ps.foreach { p =>
        raise(Evt.Blocking, 'addr -> Address("akka.gremlin.tcp", "cluster", "localhost", p))
        exec(One(Address("akka.gremlin.tcp", "cluster", "localhost", p), Drop(1, 1)))
      }
    case Unblock(ps) =>
      ps.foreach { p =>
        raise(Evt.Unblocking, 'addr -> Address("akka.gremlin.tcp", "cluster", "localhost", p))
