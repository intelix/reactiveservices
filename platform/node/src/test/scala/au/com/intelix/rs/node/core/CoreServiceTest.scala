...
import au.com.intelix.rs.core.stream.ListStreamState.{ListSpecs, RejectAdd}
...
import au.com.intelix.rs.node.testkit
import au.com.intelix.rs.node.testkit._
import au.com.intelix.rs.core.testkit.components.TestServiceActor._
import au.com.intelix.rs.core.testkit.components.TestServiceConsumer.{Close, Open, SendSignal}
...
class CoreServiceTest extends StandardMultiNodeSpec {
  ...
  trait WithClusterAwareServiceOn1 extends WithNode1 {
    ...
    override def node1Services: Map[String, Class[_]] = super.node1Services + ("test" -> classOf[au.com.intelix.rs.core.testkit.components.ClusterAwareService])
  }
  ...
  trait WithClusterAwareServiceOn2 extends WithNode2 {
    ...
    override def node2Services: Map[String, Class[_]] = super.node2Services + ("test" -> classOf[au.com.intelix.rs.core.testkit.components.ClusterAwareService])
  }
  ...
  trait WithClusterAwareServiceOn3 extends WithNode3 {
    ...
    override def node3Services: Map[String, Class[_]] = super.node3Services + ("test" -> classOf[au.com.intelix.rs.core.testkit.components.ClusterAwareService])
  }


  import au.com.intelix.rs.core.testkit.components.ClusterAwareService._
  ...
  "Cluster-aware service on single node" should "receive MemberUp notification from own node" in new WithClusterAwareServiceOn1 {
    on node1 expectOne of Evt.MemberUp + ('addr -> node1Address)
  }

  it should "become a leader" in new WithClusterAwareServiceOn1 {
    on node1 expectOne ofEach(
      Evt.LeaderChanged + ('addr -> s"Some($node1Address)"),
      Evt.LeaderTakeover)
  }
  ...
  "Cluster-aware service on two nodes" should "receive MemberUp notification from both nodes" in new WithClusterAwareServiceOn1 with WithClusterAwareServiceOn2 {
    on node1 expectOne ofEach(Evt.MemberUp + ('addr -> node1Address), Evt.MemberUp + ('addr -> node2Address))
    on node2 expectOne ofEach(Evt.MemberUp + ('addr -> node1Address), Evt.MemberUp + ('addr -> node2Address))
  }
  ...
  trait WithClusterAwareServiceRunningOnThree extends WithClusterAwareServiceRunningOnTwo with WithClusterAwareServiceOn3 {
    on node3 expectOne ofEach(Evt.MemberUp + ('addr -> node1Address), Evt.MemberUp + ('addr -> node2Address))
    clearEvents()
  }

  it should "receive Evt.MemberUnreachable followed by Evt.MemberRemoved when another node dies" in new WithClusterAwareServiceRunningOnTwo {
    stopNode1()
    on node2 expectSome of Evt.MemberUnreachable + ('addr -> node1Address)
    on node2 expectOne of Evt.MemberRemoved + ('addr -> node1Address)
  }

  it should "transfer leadership when one node dies" in new WithClusterAwareServiceRunningOnTwo {
    stopNode1()
    on node2 expectSome of Evt.LeaderChanged + ('addr -> node2Address.r)
    on node2 expectOne of Evt.LeaderTakeover
  }
  ...
  "Service running on Node1" should "start and create remote endpoints" in new With4NodesAndTestOn1 {
    on node1 expectOne of ServiceEvt.RemoteEndpointRegistered + classOf[TestServiceActor] + ('location -> node1Address)
    on node1 expectOne of ServiceEvt.RemoteEndpointRegistered + classOf[TestServiceActor] + ('location -> node2Address)
    on node1 expectOne of ServiceEvt.RemoteEndpointRegistered + classOf[TestServiceActor] + ('location -> node3Address)
    on node1 expectOne of ServiceEvt.RemoteEndpointRegistered + classOf[TestServiceActor] + ('location -> node4Address)
  }

  it should "start and create remote endpoints on nodes joined later" in new With4NodesAndTestOn1 {
    on node1 expectOne ofEach(
      ServiceEvt.RemoteEndpointRegistered + classOf[TestServiceActor] + ('location -> node1Address),
      ServiceEvt.RemoteEndpointRegistered + classOf[TestServiceActor] + ('location -> node2Address),
      ServiceEvt.RemoteEndpointRegistered + classOf[TestServiceActor] + ('location -> node3Address),
      ServiceEvt.RemoteEndpointRegistered + classOf[TestServiceActor] + ('location -> node4Address)
      )
    clearEvents()
    new WithNode5 {
      on node1 expectOne of ServiceEvt.RemoteEndpointRegistered + classOf[TestServiceActor] + ('location -> node5Address)
    }
  }

  it should "register its remote endpoints with local registries on each node" in new With4NodesAndTestOn1 {
    on node1 expectOne of ServiceRegistryActor.Evt.ServiceRegistered + ('key -> "test")
    on node2 expectOne of ServiceRegistryActor.Evt.ServiceRegistered + ('key -> "test")
    on node3 expectOne of ServiceRegistryActor.Evt.ServiceRegistered + ('key -> "test")
    on node4 expectOne of ServiceRegistryActor.Evt.ServiceRegistered + ('key -> "test")
  }
  ...
  it should "not conflict with another service with the same name started on some other node" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of ServiceRegistryActor.Evt.ServiceRegistered +('key -> "test", 'locations -> 2)
    on node2 expectOne of ServiceRegistryActor.Evt.ServiceRegistered +('key -> "test", 'locations -> 2)
    on node3 expectOne of ServiceRegistryActor.Evt.ServiceRegistered +('key -> "test", 'locations -> 2)
    on node4 expectOne of ServiceRegistryActor.Evt.ServiceRegistered +('key -> "test", 'locations -> 2)

    on node1 expectOne of ServiceRegistryActor.Evt.PublishingNewServiceLocation + ('key -> "test")
    on node2 expectOne of ServiceRegistryActor.Evt.PublishingNewServiceLocation + ('key -> "test")
    on node3 expectOne of ServiceRegistryActor.Evt.PublishingNewServiceLocation + ('key -> "test")
    on node4 expectOne of ServiceRegistryActor.Evt.PublishingNewServiceLocation + ('key -> "test")
  ...
  it should "remove remote endpoints and unregister location from local registries upon termination" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of ServiceRegistryActor.Evt.ServiceUnregistered +('key -> "test", 'remainingLocations -> 0)
    on node2 expectOne of ServiceRegistryActor.Evt.ServiceUnregistered +('key -> "test", 'remainingLocations -> 0)
    on node3 expectOne of ServiceRegistryActor.Evt.ServiceUnregistered +('key -> "test", 'remainingLocations -> 0)
    on node4 expectOne of ServiceRegistryActor.Evt.ServiceUnregistered +('key -> "test", 'remainingLocations -> 0)
  }
  ...
  it should "be able to register for other service location updates, and get update when service become available" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of TestServiceActor.Evt.OtherServiceLocationChanged +('addr -> "Some".r, 'service -> "test1")
  }
  ...
  it should "be able to register for other service location updates, and get update when service become unavailable" in new With4NodesAndTestOn1 {
    ...
    clearEvents()
    stopNode2()
    on node1 expectOne of TestServiceActor.Evt.OtherServiceLocationChanged +('addr -> "None", 'service -> "test1")
  }
  ...
  it should "not receive other service location updates if previously selected location is still active, even when new locations become available" in new With4NodesAndTestOn1 {
    ...
    new WithNode5 {
      ...
      on node5 expectOne of TestServiceActor.Evt.OtherServiceLocationChanged +('addr -> "Some".r, 'service -> "test1")
      within(3 seconds) {
        on node1 expectNone of TestServiceActor.Evt.OtherServiceLocationChanged +('addr -> "Some".r, 'service -> "test1")
      }
    }
  }
  ...
  "Service consumer" should "be able to open stream and receive an update when started on the same node" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer")
    clearEvents()
  ...
  it should "be able to open stream and receive an update when started on another node" in new With4NodesAndTestOn1 {
    ...
    on node2 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer")
    //    clearEvents()
  ...
  "Multiple service consumers" should "be able to open stream and receive an update when started on the same node" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer1")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer2")
    clearEvents()
  ...
  it should "be able to open stream and receive an update when started on different hosts" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer1")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer2")
    on node2 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer3")
    on node2 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer4")
    clearEvents()
    serviceOnNode1("consumer1") ! Open("test", "string")
    serviceOnNode1("consumer2") ! Open("test", "string")
  ...
  "Service" should "reject invalid subjects" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of ServiceEvt.SubjectMappingError + classOf[TestServiceActor]
  ...
  it should "not receive mapping request from remote endpoint if subject has already been mapped" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of ServiceEvt.SubjectMapped + classOf[TestServiceActor] + ('subj -> "test|string")

    clearEvents()
    serviceOnNode1("consumer2") ! Open("test", "string")
    on node1 expectOne of TestServiceConsumer.Evt.StringUpdate +('path -> "/user/node/consumer2", 'sourceService -> "test", 'topic -> "string", 'value -> TestServiceActor.AutoStringReply)

    on node1 expectNone of ServiceEvt.SubjectMapped + classOf[TestServiceActor] + ('subj -> "test|string")
  }
  ...
  it should "close stream when all subjects mapped to the stream have been discarded" in new With4NodesAndTestOn1 {
    ...
    within(5 seconds) {
      on node1 expectNone of ServiceEvt.IdleStream + classOf[TestServiceActor]
    }
    clearEvents()

    serviceOnNode1("consumer2") ! Close("test", "string2")
    on node1 expectOne of ServiceEvt.IdleStream + classOf[TestServiceActor]
  ...
  it should "support compound stream id" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of TestServiceConsumer.Evt.StringUpdate +('path -> "/user/node/consumer1", 'sourceService -> "test", 'topic -> "stringWithId", 'keys -> UserId(1), 'value -> TestServiceActor.AutoStringReply)
    on node1 expectOne of TestServiceConsumer.Evt.StringUpdate +('path -> "/user/node/consumer1", 'sourceService -> "test", 'topic -> "stringWithId", 'keys -> UserId(2), 'value -> TestServiceActor.AutoStringReply)

    clearEvents()

    serviceOnNode1("test") ! PublishString(CompoundStreamId("string", 1), "latest")

    on node1 expectOne of TestServiceConsumer.Evt.StringUpdate +('path -> "/user/node/consumer1", 'sourceService -> "test", 'topic -> "stringWithId", 'keys -> UserId(1), 'value -> "latest")
    within(3 seconds) {
      on node1 expectNone of TestServiceConsumer.Evt.StringUpdate +('path -> "/user/node/consumer1", 'sourceService -> "test", 'topic -> "stringWithId", 'keys -> UserId(2), 'value -> "latest")
  ...
  it should "notify when stream becomes active" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of TestServiceActor.Evt.StreamActive + ('stream -> "string#1")
  ...
  it should "notify when stream becomes passive" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of TestServiceActor.Evt.StreamPassive + ('stream -> "string#1")
  }
  ...
  it should "shut down stream at location when all consumers are gone from that location" in new ServiceWith4ActiveConsumers {
    ...
    on node1 expectOne of ServiceEvt.StreamInterestRemoved + classOf[TestServiceActor] + ('location -> node2Address)
  }
  ...
  trait With3Consumers1Service extends With4NodesAndTestOn1 {
    ...
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer1")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer2")
    on node3 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "consumer3")
    clearEvents()
  }

  it should "receive a signal" in new With3Consumers1Service {
    serviceOnNode3("consumer3") ! SendSignal("test", "signal")
    on node3 expectOne of TestServiceConsumer.Evt.SignalResponseReceivedAckOk + ('path -> "/user/node/consumer3")
    on node3 expectOne of TestServiceConsumer.Evt.SignalResponseReceivedAckOk
  }
  it should "receive a signal with payload" in new With3Consumers1Service {
    ...
    on node3 expectOne of TestServiceConsumer.Evt.SignalResponseReceivedAckOk +('payload -> "Some(payload1)", 'path -> "/user/node/consumer3")
  }
  it should "receive a signal with payload and correlationId" in new With3Consumers1Service {
    ...
    on node3 expectOne of TestServiceConsumer.Evt.SignalResponseReceivedAckOk +('payload -> "Some(payload1)", 'correlationId -> "Some(correlationId)", 'path -> "/user/node/consumer3")
  }
  it should "receive a signal with payload and correlationId, ordered group" in new With3Consumers1Service {
    ...
    for (i <- 1 to 100) on node3 expectOne of TestServiceConsumer.Evt.SignalResponseReceivedAckOk +('payload -> s"Some(payload$i:$i)", 'correlationId -> s"Some(correlation$i)", 'path -> "/user/node/consumer3")
  }
  it should "receive a signal and respond with a failure" in new With3Consumers1Service {
    ...
    on node3 expectOne of TestServiceConsumer.Evt.SignalResponseReceivedAckFailed +('payload -> "Some(failure)", 'path -> "/user/node/consumer3")
  }
  it should "receive a signal and do not respond" in new With3Consumers1Service {
    ...
    on node1 expectOne of TestServiceActor.Evt.SignalReceived + ('subj -> "test|signal_no_response")
    within(3 seconds) {
      on node3 expectNone of TestServiceConsumer.Evt.SignalResponseReceivedAckOk + ('path -> "/user/node/consumer3")
      on node3 expectNone of TestServiceConsumer.Evt.SignalResponseReceivedAckFailed + ('path -> "/user/node/consumer3")
    }
  }
  ...
  it should "receive a signal and do not respond, client should timeout" in new With3Consumers1Service {
    ...
    on node3 expectOne of TestServiceConsumer.Evt.SignalTimeout + ('path -> "/user/node/consumer3")
  ...
  it should "may or may not receive all updates, but must receive the very last one" in new With3Consumers1Service {
    ...
    serviceOnNode1("test") ! PublishString("string", "update2")
    serviceOnNode1("test") ! PublishString("string", "update3")
    on node3 expectOne of TestServiceConsumer.Evt.StringUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "string", 'value -> "update3")
  ...
  "Set stream subscriber" should "receive an initial update when subscribed, followed by all consequent updates" in new With3Consumers1Service {
    ...
    on node3 expectSome of TestServiceConsumer.Evt.SetUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "set", 'value -> TestServiceActor.AutoSetReply.mkString(","))
    clearEvents()

    serviceOnNode1("test") ! PublishSet("set", Set("c", "d"))
    on node3 expectSome of TestServiceConsumer.Evt.SetUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "set", 'value -> "c,d")
    clearEvents()

    serviceOnNode1("test") ! PublishSetAdd("set", Set("c", "x"))
    on node3 expectSome of TestServiceConsumer.Evt.SetUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "set", 'value -> "c,d,x")
    clearEvents()

    serviceOnNode1("test") ! PublishSetRemove("set", Set("c", "a"))
    on node3 expectSome of TestServiceConsumer.Evt.SetUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "set", 'value -> "d,x")
  ...
  it should "not conflict with string subscription" in new With3Consumers1Service {
    ...
    on node3 expectNone of TestServiceConsumer.Evt.StringUpdate
  ...
  "Map stream subscriber" should "receive an initial update when subscribed, followed by all consequent updates" in new With3Consumers1Service {
    ...
    on node3 expectSome of TestServiceConsumer.Evt.MapUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "map", 'value -> "Map(s -> a, i -> 1, b -> true)")
    clearEvents()

    serviceOnNode1("test") ! PublishMap("map", Array("c", 123, false))
    on node3 expectSome of TestServiceConsumer.Evt.MapUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "map", 'value -> "Map(s -> c, i -> 123, b -> false)")
    clearEvents()

    serviceOnNode1("test") ! PublishMap("map", Array("c", 123, true))
    on node3 expectSome of TestServiceConsumer.Evt.MapUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "map", 'value -> "Map(s -> c, i -> 123, b -> true)")
    clearEvents()

    serviceOnNode1("test") ! PublishMapAdd("map", "s" -> "bla")
    on node3 expectSome of TestServiceConsumer.Evt.MapUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "map", 'value -> "Map(s -> bla, i -> 123, b -> true)")
    clearEvents()
  ...
  "List stream subscriber" should "receive an initial update when subscribed, followed by all consequent updates" taggedAs (OnlyThisTest)  in new With3Consumers1Service {
    ...
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list1", 'value -> "1,2,3,4")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2", 'value -> "1,2,3,4")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list3", 'value -> "1,2,3,4")
    clearEvents()

    serviceOnNode1("test") ! PublishList("list1", List("a", "b"), ListSpecs(5, RejectAdd))
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list1", 'value -> "a,b")
    clearEvents()

    serviceOnNode1("test") ! PublishListAdd("list1", -1, "5")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list1", 'value -> "a,b,5")
    clearEvents()

    serviceOnNode1("test") ! PublishListAdd("list1", 0, "6")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list1", 'value -> "6,a,b,5")
    clearEvents()

    serviceOnNode1("test") ! PublishListAdd("list1", 2, "7")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list1", 'value -> "6,a,7,b,5")
    clearEvents()
    ...
    serviceOnNode1("test") ! PublishListAdd("list1", 2, "8")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list1", 'value -> "a,b,8,c,d,e")
    clearEvents()


    serviceOnNode1("test") ! PublishListAdd("list2", -1, "5")
    serviceOnNode1("test") ! PublishListAdd("list3", -1, "5")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2", 'value -> "1,2,3,4,5")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list3", 'value -> "1,2,3,4,5")
    clearEvents()

    serviceOnNode1("test") ! PublishListAdd("list2", 0, "6")
    serviceOnNode1("test") ! PublishListAdd("list3", 0, "6")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2", 'value -> "1,2,3,4,5")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list3", 'value -> "6,1,2,3,4")
    clearEvents()

    serviceOnNode1("test") ! PublishListAdd("list2", 2, "7")
    serviceOnNode1("test") ! PublishListAdd("list3", 2, "7")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2", 'value -> "2,7,3,4,5")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list3", 'value -> "6,1,7,2,3")
    clearEvents()

    serviceOnNode1("test") ! PublishListRemove("list2", 2)
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2", 'value -> "2,7,4,5")
    clearEvents()

    serviceOnNode1("test") ! PublishListReplace("list2", -2, "x")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2", 'value -> "2,7,x,5")
    clearEvents()

    serviceOnNode1("test") ! PublishListFindReplace("list2", "7", "x")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2", 'value -> "2,x,x,5")
    clearEvents()

    serviceOnNode1("test") ! PublishListFindReplace("list2", "x", "a")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2", 'value -> "2,a,x,5")
    clearEvents()

    serviceOnNode1("test") ! PublishListFindRemove("list2", "z")
    within(1 seconds) {
      on node3 expectNone of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2")
    }
    clearEvents()

    serviceOnNode1("test") ! PublishListFindRemove("list2", "a")
    on node3 expectSome of TestServiceConsumer.Evt.ListUpdate +('path -> "/user/node/consumer3", 'sourceService -> "test", 'topic -> "list2", 'value -> "2,x,5")
    clearEvents()
