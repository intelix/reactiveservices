...
object EvtContext {
  ...
  def apply(evtSourceId: String, cfg: Config, staticFields: (Symbol, Any)*): EvtContext = new EvtContext {
    ...
    cfg match {
      case t: CommonAttributes => addEvtFields(t.commonAttributes.toList: _*)
      case _ =>
    }
  ...
  def raiseWith[T](c: MyContext)(e: c.Expr[Evt], fields: c.Expr[EvtFieldValue]*)(f: c.Expr[EvtFieldBuilder => T]) = {
    ...
    q"""
        if ($pub.canPublish($src, $e)) {
          val b = new au.com.intelix.evt.EvtFieldBuilderWithList($initialList)
          val result = $f(b)
          $pub.raise ($src, $e, b.result)
          result
        } else {
          $f(au.com.intelix.evt.MuteEvtFieldBuilder)
        }
      """
