...
import au.com.intelix.rs.auth.{AuthServiceActor, AuthStage}
...
import au.com.intelix.rs.node.core.ClusterNodeActor
import au.com.intelix.rs.node.testkit.StandardMultiNodeSpec
import au.com.intelix.rs.websocket.WebSocketClient.Evt
import au.com.intelix.rs.websocket.WebsocketClientStubService._
...
class WebsocketTest extends StandardMultiNodeSpec {
  ...
  trait With4NodesAndTestOn1 extends With4Nodes {
    ...
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "client")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "websocket-server")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "test")
    on node2 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "test2")
  ...
  "Websocket client" should "successfully connect to the endpoint and upgrade to websocket" in new With4NodesAndTestOn1 {
    ...
    on node1 expectOne of Evt.ConnectionUpgraded
  }
  ...
  it should "subscribe to a stream" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "hello")
  }
  ...
  it should "receive updates when service posts them" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "update1")
  }
  ...
  it should "receive updates only for the subscribed streams" in new WithClientConnected {
    ...
    within(3 seconds) {
      on node1 expectNone of Evt.StringUpdate
      on node1 expectNone of Evt.MapUpdate
    }
  }
  ...
  it should "receive notification when service becomes unavailable" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.ReceivedServiceNotAvailable
  }
  ...
  it should "receive notification when service becomes available" in new WithClientConnected {
    ...
    within(2 seconds) {
      on node1 expectNone of Evt.StringUpdate +('sourceService -> "test3", 'topic -> "string", 'id -> "c1", 'value -> "hello")
    }
    clearEvents()
    ...
    new WithNode5 {
      ...
      on node5 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "test3")
      on node1 expectSome of Evt.StringUpdate +('sourceService -> "test3", 'topic -> "string", 'id -> "c1", 'value -> "hello")
    }
  ...
  it should "receive notification when service becomes unavailable and then receive an update when service becomes available again" in new WithClientConnected {
    serviceOnNode1("client/c1") ! OpenSubscriptionFromStub(Subject("test2", "string"))
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test2", 'topic -> "string", 'id -> "c1", 'value -> "hello")
    serviceOnNode2("test2") ! PublishString("string", "update1")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test2", 'topic -> "string", 'id -> "c1", 'value -> "update1")
    clearEvents()
    ...
    new WithNode5 {
      ...
      on node5 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "test2")
      on node1 expectSome of Evt.StringUpdate +('sourceService -> "test2", 'topic -> "string", 'id -> "c1", 'value -> "hello")
    }
  ...
  it should "be able to subscribe to multiple streams and receive corresponding updates" in new WithClientConnected {
    ...
    on node1 expectSome ofEach(
      Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "hello"),
      Evt.StringUpdate +('topic -> "string1", 'id -> "c1", 'value -> "hello"),
      Evt.StringUpdate +('topic -> "stringX", 'id -> "c1", 'value -> "helloX")
      )
    ...
    serviceOnNode1("test") ! PublishString("string", "update1")
    on node1 expectSome of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "update1")
    on node1 expectSome of Evt.StringUpdate +('topic -> "string1", 'id -> "c1", 'value -> "update1")

    serviceOnNode1("test") ! PublishString("stringX", "update1X")
    on node1 expectSome of Evt.StringUpdate +('topic -> "stringX", 'id -> "c1", 'value -> "update1X")
  }
  ...
  it should "be able to drop subscription and no longer receive updates" in new WithClientConnected {
    ...
    on node1 expectOne of ServiceEvt.IdleStream + classOf[TestServiceActor] + ('stream -> "string")
  ...
  it should "be able to drop subscription and this should not affect any other active streams" in new WithClientConnected {
    ...
    serviceOnNode1("client/c1") ! OpenSubscriptionFromStub(Subject("test", "stringX"))
    on node1 expectSome of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "hello")
    on node1 expectSome of Evt.StringUpdate +('topic -> "string1", 'id -> "c1", 'value -> "hello")
    on node1 expectSome of Evt.StringUpdate +('topic -> "stringX", 'id -> "c1", 'value -> "helloX")
    clearEvents()
    ...
    on node1 expectSome of Evt.StringUpdate +('topic -> "string1", 'id -> "c1", 'value -> "update1")
    on node1 expectSome of Evt.StringUpdate +('topic -> "stringX", 'id -> "c1", 'value -> "updateX")
    within(2 seconds) {
      on node1 expectNone of Evt.StringUpdate +('topic -> "string", 'id -> "c1")
    }
  ...
  it should "receive updates for Map stream" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.MapUpdate +('topic -> "map", 'id -> "c1", 'value -> "Map(s -> a, i -> 1, b -> true)")
    serviceOnNode1("test") ! PublishMapAdd("map", "s" -> "b")
    on node1 expectSome of Evt.MapUpdate +('topic -> "map", 'id -> "c1", 'value -> "Map(s -> b, i -> 1, b -> true)")
  }
  ...
  it should "receive updates for Set stream" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.SetUpdate +('topic -> "set", 'id -> "c1", 'value -> "a,b")
    serviceOnNode1("test") ! PublishSetAdd("set", Set("x", "y"))
    on node1 expectSome of Evt.SetUpdate +('topic -> "set", 'id -> "c1", 'value -> "a,b,x,y")
  }
  ...
  it should "receive updates for List stream" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.ListUpdate +('topic -> "list1", 'id -> "c1", 'value -> "1,2,3,4")
    serviceOnNode1("test") ! PublishListAdd("list1", 0, "x")
    on node1 expectSome of Evt.ListUpdate +('topic -> "list1", 'id -> "c1", 'value -> "x,1,2,3,4")
  }
  ...
  it should "receive partial Map updates when enabled" in new WithClientConnected {
    ...
    on node1 expectOne of Evt.ReceivedStreamStateTransitionUpdate + ('transition -> "Partial.+b,NoChange,NoChange".r)
  ...
  it should "receive partial List updates when enabled" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.ListUpdate +('topic -> "list1", 'id -> "c1", 'value -> "x,1,2,3,4")
    on node1 expectSome of Evt.ListUpdate +('topic -> "list1", 'id -> "c1", 'value -> "y,1,2,3,4")

    on node1 expectSome of Evt.ReceivedStreamStateTransitionUpdate + ('transition -> "Partial.+Replace[(]0,y[)]".r)
  ...
  it should "receive partial Set updates when enabled" in new WithClientConnected {
    ...
    serviceOnNode1("test") ! PublishSetAdd("set", Set("x", "y"))
    on node1 expectSome of Evt.SetUpdate +('topic -> "set", 'id -> "c1", 'value -> "a,b,x,y")
    on node1 expectSome of Evt.ReceivedStreamStateTransitionUpdate + ('transition -> "Partial.+Add[(]y[)]".r)
  ...
  it should "be able to reset the stream" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.MapUpdate +('topic -> "map", 'id -> "c1", 'value -> "Map(s -> b, i -> 1, b -> true)")
    on node1 expectOne of Evt.ReceivedStreamStateUpdate + ('state -> "s=b,i=1,b=true".r)
  ...
  it should "receive updates with server-side aggregation enabled" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "update2")
    on node1 expectSome of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "update3")
    on node1 expectSome of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "update4")
  }
  ...
  it should "receive pings when enabled" in new WithClientConnected {
    ...
    serviceOnNode1("client/c1") ! OpenSubscriptionFromStub(Subject("test", "string"))
    on node1 expectSome of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "hello")
    on node1 expectSome of Evt.ReceivedPing
  }
  ...
  /** TODO - check this one
    ...
    * on node1 expectOne of TestServiceActorEvt.EvtIdleStream + ('stream -> "string")
    * }
    */
  ...
  it should "send a signal and receive a successful ack" in new WithClientConnected {
    ...
    on node1 expectSome of Evt.ReceivedSignalAckOk +('correlation -> "Some(cId)", 'payload -> "Some(value1)", 'id -> "c1")
  }
  ...
  it should "send a signal and receive a failure ack " in new WithClientConnected {
    ...
    on node1 expectSome of Evt.ReceivedSignalAckFailed +('correlation -> "Some(cId)", 'payload -> "Some(failure)", 'id -> "c1")
  }
  ...
  it should "send a signal and receive a timeout " in new WithClientConnected {
    ...
    on node1 expectSome of Evt.ReceivedSignalAckFailed +('correlation -> "Some(cId)", 'payload -> "None", 'id -> "c1")
  }
  ...
  it should "send multiple signals and receive successful ack for all, ordered" in new WithClientConnected {
    ...
    for (i <- 1 to 100) on node1 expectSome of Evt.ReceivedSignalAckOk +('correlation -> s"Some(cId:$i)", 'payload -> s"Some(value-$i:$i)", 'id -> "c1")
  }
  ...
  it should "receive updates when subscribed with aggregation for a specific stream" in new WithClientConnected {
    ...
    serviceOnNode1("test") ! PublishString("string", "update4")
    on node1 expectSome of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "update4")
    on node1 expectNone of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "update2")
    on node1 expectNone of Evt.StringUpdate +('topic -> "string", 'id -> "c1", 'value -> "update3")
  }
  ...
  trait With3Clients extends With4Nodes {
    ...
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "client")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "client2")
    on node3 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "client3")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "websocket-server")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "test")
    on node2 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "test2")
  ...
  "Multiple websocket clients" should "successfully connect to the same endpoint and upgrade to websocket" in new With3Clients {
    ...
    on node1 expectOne of Evt.ConnectionUpgraded + ('id -> "c1")
    on node1 expectOne of Evt.ConnectionUpgraded + ('id -> "c2")
    on node3 expectOne of Evt.ConnectionUpgraded + ('id -> "c3")
  }
  ...
  it should "subscribe to different streams and receive updates" in new With3ClientsConnected {
    ...
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "string", 'id -> "c1", 'value -> "hello")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test2", 'topic -> "stringX", 'id -> "c2", 'value -> "helloX")
    on node3 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringX", 'id -> "c3", 'value -> "helloX")

    clearEvents()

    serviceOnNode1("test") ! PublishString("string", "update1")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "string", 'id -> "c1", 'value -> "update1")

    serviceOnNode2("test2") ! PublishString("stringX", "update2")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test2", 'topic -> "stringX", 'id -> "c2", 'value -> "update2")

    serviceOnNode1("test") ! PublishString("stringX", "update3")
    on node3 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringX", 'id -> "c3", 'value -> "update3")
  ...
  it should "subscribe to same stream and receive updates" in new With3ClientsConnected {
    ...
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "string", 'id -> "c1", 'value -> "hello")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "string", 'id -> "c2", 'value -> "hello")
    on node3 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "string", 'id -> "c3", 'value -> "hello")

    clearEvents()

    serviceOnNode1("test") ! PublishString("string", "update1")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "string", 'id -> "c1", 'value -> "update1")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "string", 'id -> "c2", 'value -> "update1")
    on node3 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "string", 'id -> "c3", 'value -> "update1")
  }
  ...
  it should "not be affected by some client dropping" in new With3ClientsConnected {
    ...
    serviceOnNode1("client") ! StopRequest
    on node1 expectSome of CommonActorEvt.Evt.PostStop + ('id -> "c1")
    clearEvents()
  ...
  it should "be able to connect to different endpoint, subscribe to stream and get updates" in new With4Nodes {
    ...
    on node3 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "client3")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "websocket-server")
    on node2 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "websocket-server")
    on node3 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "websocket-server")
    on node1 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "test")
    on node2 expectOne of ClusterNodeActor.Evt.StartingService + ('service -> "test2")
    clearEvents()
  ...
  "With auth enabled, multiple clients" should "not be able to access any services without auth" in new With3ClientsConnectedAuthEnabled {
    ...
    on node1 expect(1 to 3) of AuthStage.Evt.AccessDenied + ('subj -> "test|string")
  }
  ...
  it should "be able to authenticate with credentials" in new With3ClientsConnectedAuthEnabled {
    ...
    on node1 expectSome of Evt.ReceivedSignalAckOk +('correlation -> "Some(auth1)", 'payload -> "Some(true)", 'id -> "c1")
    on node1 expectSome of Evt.ReceivedSignalAckOk +('correlation -> "Some(auth2)", 'payload -> "Some(true)", 'id -> "c2")
    on node3 expectSome of Evt.ReceivedSignalAckOk +('correlation -> "Some(auth3)", 'payload -> "Some(true)", 'id -> "c3")

    on node1 expect(2) of AuthServiceActor.Evt.SuccessfulCredentialsAuth + ('user -> "user1")
    on node1 expect(1) of AuthServiceActor.Evt.SuccessfulCredentialsAuth + ('user -> "user2")

    clearEvents()

    serviceOnNode1("client/c1") ! OpenSubscriptionFromStub(Subject("auth", "token"))
    on node1 expectOne of Evt.StringUpdate +('sourceService -> "auth", 'topic -> "token")
    val auth1Token = locateFirstEventFieldValue[String](Evt.StringUpdate, "value")
    ...
    on node1 expectOne of Evt.StringUpdate +('sourceService -> "auth", 'topic -> "token")
    val auth2Token = locateFirstEventFieldValue[String](Evt.StringUpdate, "value")


    clearEvents()

    serviceOnNode3("client3/c3") ! OpenSubscriptionFromStub(Subject("auth", "token"))
    on node3 expectOne of Evt.StringUpdate +('sourceService -> "auth", 'topic -> "token", 'value -> auth1Token)
    // same token as the same user
    val auth3Token = locateFirstEventFieldValue[String](Evt.StringUpdate, "value")
  ...
  trait With3AuthenticatedClients extends With3ClientsConnectedAuthEnabled {
    ...
    serviceOnNode1("client/c1") ! OpenSubscriptionFromStub(Subject("auth", "token"))
    on node1 expectOne of Evt.StringUpdate +('sourceService -> "auth", 'topic -> "token")
    val auth1Token = locateFirstEventFieldValue[String](Evt.StringUpdate, "value").asInstanceOf[String]
    ...
    serviceOnNode1("client2/c2") ! OpenSubscriptionFromStub(Subject("auth", "token"))
    on node1 expectOne of Evt.StringUpdate +('sourceService -> "auth", 'topic -> "token")
    val auth2Token = locateFirstEventFieldValue[String](Evt.StringUpdate, "value").asInstanceOf[String]
    ...
    on node3 expectOne of Evt.StringUpdate +('sourceService -> "auth", 'topic -> "token", 'value -> auth1Token)
    // same token as the same user
    val auth3Token = locateFirstEventFieldValue[String](Evt.StringUpdate, "value").asInstanceOf[String]
  ...
  it should "be able to re-authenticate using token" in new With3AuthenticatedClients {
    serviceOnNode1("client/c1") ! SignalFromStub(
      ...
      auth1Token,
      System.currentTimeMillis() + 8000, None, Some("auth1"))
    on node1 expectOne of AuthServiceActor.Evt.SuccessfulTokenAuth + ('authkey -> auth1Token.r)
    on node1 expectSome of Evt.ReceivedSignalAckOk +('correlation -> "Some(auth1)", 'payload -> "Some(true)", 'id -> "c1")
  }
  ...
  it should "be able to subscribe to user-specific stream and receive independent updates" in new With3AuthenticatedClients {
    ...
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringWithId", 'id -> "c1", 'value -> "hello")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringWithId", 'id -> "c2", 'value -> "hello")
    on node3 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringWithId", 'id -> "c3", 'value -> "hello")

    clearEvents()

    serviceOnNode1("test") ! PublishString(CompoundStreamId("string", 1), "update-user1")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringWithId", 'id -> "c1", 'value -> "update-user1")
    on node3 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringWithId", 'id -> "c3", 'value -> "update-user1")

    within(2 seconds) {
      on node1 expectNone of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringWithId", 'id -> "c2")
    }

    clearEvents()

    serviceOnNode1("test") ! PublishString(CompoundStreamId("string", 2), "update-user2")
    on node1 expectSome of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringWithId", 'id -> "c2", 'value -> "update-user2")

    within(2 seconds) {
      on node1 expectNone of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringWithId", 'id -> "c1")
      on node3 expectNone of Evt.StringUpdate +('sourceService -> "test", 'topic -> "stringWithId", 'id -> "c3")
    }
