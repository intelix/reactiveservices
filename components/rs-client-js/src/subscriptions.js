...
function _subscribeToSharedStream(service, topic, priority, throttling, callbacks) {
  ...
  if (!subscribers) {
    subscribers = [];
    if (Log.isDebug()) Log.logDebug(componentId, "Server subscription for: " + key + ", alias: " + alias);
  ...
  /// !>>>> TEST
  if (pendingDataRequests && pendingDataRequests.length > 0)     _processPendingDataRequests();
  ...
  if (subjectKey) {
    ...
    if (subscribers && subscribers.length > 0) {
      subscribers.forEach(function (next) {
        // console.debug(`!>>>>> Updating ${alias} - ${subjectKey}`);
        next.onUpdate(data);
      });
    }
  } else {
