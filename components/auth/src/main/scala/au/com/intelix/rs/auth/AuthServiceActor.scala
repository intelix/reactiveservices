...
import au.com.intelix.rs.core.SubjectTags.UserToken
...
import au.com.intelix.rs.auth.api.AuthenticationMessages.{Authenticate, AuthenticationResponse, Invalidate}
import au.com.intelix.rs.auth.api.AuthorisationMessages.{DomainPermissions, PermissionsRequest, PermissionsRequestCancel, SubjectPermissions}
...
object AuthServiceActor {
  ...
  object Evt {
    case object AuthToken extends TraceE
    case object UserInfo extends TraceE
    case object UserDomainPermissions extends TraceE
    case object UserSubjectsPermissions extends TraceE
    case object UserTokenAdded extends InfoE
    case object SessionCreated extends InfoE
    case object UserSessionExpired extends InfoE
    case object UserSessionInvalidated extends InfoE
    case object AuthRequest extends InfoE
    case object SuccessfulCredentialsAuth extends InfoE
    case object SuccessfulTokenAuth extends InfoE
    case object FailedCredentialsAuth extends InfoE
    case object FailedTokenAuth extends InfoE
  }
...
class AuthServiceActor extends StatelessServiceActor {
  ...
  onSignalAsync {
    case (Subject(_, TopicKey("cauth"), UserToken(ut)), body: String) =>
      raise(Evt.AuthRequest, 'token -> ut)
      parseCredentials(body) match {
        case None =>
          raise(Evt.FailedCredentialsAuth, 'reason -> "invalid request")
  ...
  onSignal {
    ...
    case (Subject(_, TopicKey("tauth"), UserToken(ut)), securityToken: String) =>
      raise(Evt.AuthRequest, 'token -> ut, 'authkey -> securityToken)
      authenticateWithToken(securityToken, ut) match {
        case Some(sess) =>
          raise(Evt.SuccessfulTokenAuth, 'token -> ut, 'authkey -> securityToken, 'user -> sess.user, 'userid -> sess.userId)
          publishAllForUserToken(ut)
          SignalOk(true)
        case _ =>
          raise(Evt.FailedTokenAuth, 'token -> ut, 'authkey -> securityToken, 'reason -> "access denied")
          SignalOk(false)
      }
  ...
  onMessage {
    ...
    case AuthOk(ut, user, userId, tags) =>
      ...
      raise(Evt.SuccessfulCredentialsAuth, 'token -> ut, 'authkey -> sess.securityToken, 'user -> user, 'userid -> userId, 'tags -> tags)
      publishAllForUserToken(ut)
    case AuthFailed(ut, user) =>
      raise(Evt.FailedCredentialsAuth, 'token -> ut, 'user -> user, 'reason -> "access denied")
  }
  ...
  def invalidateSession(sess: Session) = {
    ...
    raise(Evt.UserSessionInvalidated, 'user -> sess.user, 'userid -> sess.userId, 'authkey -> sess.securityToken)
  }
  ...
  def sessionsHousekeeping() = sessions = sessions filter {
    case (_, s@Session(ut, uname, uid, tags, at, Some(t), _, _)) if now - t > SessionTimeout.toMillis =>
      raise(Evt.UserSessionExpired, 'user -> uname, 'userid -> uid, 'authkey -> at)
  ...
  def publishToken(userToken: String): Unit = {
    ...
    raise(Evt.AuthToken, 'token -> userToken, 'authkey -> securityToken)
  }
  ...
  def publishInfo(userToken: String): Unit = {
    ...
    raise(Evt.UserInfo, 'token -> userToken, 'user -> username, 'userid -> userId, 'tags -> tags)
  }
  ...
  def publishDomainPermissions(userToken: String): Unit = {
    ...
    raise(Evt.UserDomainPermissions, 'token -> userToken, 'set -> set)
  }
  ...
  def publishSubjectPermissions(userToken: String): Unit = {
    ...
    raise(Evt.UserSubjectsPermissions, 'token -> userToken, 'set -> set)
  }
  ...
  def createSession(ut: String, user: String, userId: Int, tags: Option[String]): Session =
    sessionByUserToken(ut) orElse sessionByUserId(userId) match {
      case None =>
        val newSess = Session(Set(ut), user, userId, tags, generateUUID, None)
        sessions += userId -> newSess
        authorisationProviderRef ! PermissionsRequest(userId)
        raise(Evt.SessionCreated, 'token -> ut, 'user -> user, 'userid -> userId, 'authkey -> newSess.securityToken, 'tags -> tags)
        newSess
  ...
  def addUserToken(session: Session, ut: String) = {
    ...
    raise(Evt.UserTokenAdded, 'token -> ut, 'user -> newSess.user, 'userid -> newSess.userId, 'authkey -> newSess.securityToken)
    newSess
  }
